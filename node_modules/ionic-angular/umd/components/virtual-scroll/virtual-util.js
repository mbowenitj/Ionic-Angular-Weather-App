(function (factory) {
    ...
    function processRecords(stopAtHeight, records, cells, headerFn, footerFn, data) {
        ...
        var /** @type {?} */ tmpData;
        var /** @type {?} */ lastRecordIndex = (records.length - 1);
        if (cells.length) {
    ...
    function updateDimensions(plt, nodes, cells, data, initialUpdate) {
        ...
        for (var /** @type {?} */ i = 0; i < totalCells; i++) {
            cell = cells[i];
    ...
    exports.updateNodeContext = updateNodeContext;
    /**
     * DOM READ
     * @param {?} plt
     * @param {?} cell
    ...
    function adjustRendered(cells, data) {
        ...
        if (data.scrollDiff > 0) {
            ...
            for (var /** @type {?} */ i = data.bottomCell; i >= 0; i--) {
                cell = cells[i];
                if (cell.row !== lastRow) {
                    cellsRenderHeight += cell.height;
                    lastRow = cell.row;
